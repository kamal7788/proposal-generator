...
for MIGRATION in \
  ...
  "DO \$\$ BEGIN IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='proposals' AND column_name='lighthouseAgenticBrowsing') THEN ALTER TABLE proposals ADD COLUMN \"lighthouseAgenticBrowsing\" INTEGER; END IF; EXCEPTION WHEN OTHERS THEN NULL; END \$\$" \
