...
for MIGRATION in \
  ...
  "DO \$\$ BEGIN IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='proposals' AND column_name='lighthouseAgenticBrowsing') THEN ALTER TABLE proposals ADD COLUMN \"lighthouseAgenticBrowsing\" TEXT; END IF; EXCEPTION WHEN OTHERS THEN NULL; END \$\$" \
  "DO \$\$ BEGIN IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='proposals' AND column_name='lighthouseAgenticBrowsing' AND data_type='integer') THEN ALTER TABLE proposals ALTER COLUMN \"lighthouseAgenticBrowsing\" TYPE TEXT USING \"lighthouseAgenticBrowsing\"::TEXT; END IF; EXCEPTION WHEN OTHERS THEN NULL; END \$\$" \
  ...
  "DO \$\$ BEGIN IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='proposals' AND column_name='internalNotes') THEN ALTER TABLE proposals ADD COLUMN \"internalNotes\" TEXT; END IF; EXCEPTION WHEN OTHERS THEN NULL; END \$\$" \
  "DO \$\$ BEGIN IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='users' AND column_name='role') THEN ALTER TABLE users ADD COLUMN role TEXT NOT NULL DEFAULT 'admin'; END IF; EXCEPTION WHEN OTHERS THEN NULL; END \$\$" \
  "DO \$\$ BEGIN IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='proposals' AND column_name='hasWebsite') THEN ALTER TABLE proposals ADD COLUMN \"hasWebsite\" BOOLEAN NOT NULL DEFAULT true; END IF; EXCEPTION WHEN OTHERS THEN NULL; END \$\$" \
  "DO \$\$ BEGIN IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='proposals' AND column_name='avgCustomerSpend') THEN ALTER TABLE proposals ADD COLUMN \"avgCustomerSpend\" DOUBLE PRECISION; END IF; EXCEPTION WHEN OTHERS THEN NULL; END \$\$" \
  "DO \$\$ BEGIN IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='proposals' AND column_name='customersPerDay') THEN ALTER TABLE proposals ADD COLUMN \"customersPerDay\" INTEGER; END IF; EXCEPTION WHEN OTHERS THEN NULL; END \$\$" \
  "DO \$\$ BEGIN IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='proposals' AND column_name='workingDaysPerMonth') THEN ALTER TABLE proposals ADD COLUMN \"workingDaysPerMonth\" INTEGER DEFAULT 26; END IF; EXCEPTION WHEN OTHERS THEN NULL; END \$\$" \
  "CREATE TABLE IF NOT EXISTS password_reset_tokens (id TEXT PRIMARY KEY, token TEXT UNIQUE NOT NULL, \"userId\" TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE, expires TIMESTAMP NOT NULL, used BOOLEAN NOT NULL DEFAULT false, \"createdAt\" TIMESTAMP NOT NULL DEFAULT now())" \
  "CREATE TABLE IF NOT EXISTS pricing_packages (id TEXT PRIMARY KEY, \"proposalId\" TEXT NOT NULL REFERENCES proposals(id) ON DELETE CASCADE, name TEXT NOT NULL, description TEXT, price DOUBLE PRECISION NOT NULL, \"billingPeriod\" TEXT NOT NULL DEFAULT 'monthly', \"includedServiceIds\" JSONB, \"isDefault\" BOOLEAN NOT NULL DEFAULT false, \"sortOrder\" INTEGER NOT NULL DEFAULT 0, \"createdAt\" TIMESTAMP NOT NULL DEFAULT now(), \"updatedAt\" TIMESTAMP NOT NULL DEFAULT now())" \
  "CREATE TABLE IF NOT EXISTS pricing_addons (id TEXT PRIMARY KEY, \"proposalId\" TEXT NOT NULL REFERENCES proposals(id) ON DELETE CASCADE, name TEXT NOT NULL, description TEXT, price DOUBLE PRECISION NOT NULL, \"billingPeriod\" TEXT NOT NULL DEFAULT 'monthly', \"sortOrder\" INTEGER NOT NULL DEFAULT 0, \"createdAt\" TIMESTAMP NOT NULL DEFAULT now(), \"updatedAt\" TIMESTAMP NOT NULL DEFAULT now())" \
  "CREATE TABLE IF NOT EXISTS proposal_case_studies (id TEXT PRIMARY KEY, \"proposalId\" TEXT NOT NULL REFERENCES proposals(id) ON DELETE CASCADE, \"caseStudyId\" TEXT NOT NULL REFERENCES case_studies(id) ON DELETE CASCADE, \"overriddenFields\" JSONB, \"sortOrder\" INTEGER NOT NULL DEFAULT 0, \"createdAt\" TIMESTAMP NOT NULL DEFAULT now(), UNIQUE(\"proposalId\", \"caseStudyId\"))" \
  "CREATE TABLE IF NOT EXISTS competitor_snapshots (id TEXT PRIMARY KEY, \"proposalId\" TEXT NOT NULL REFERENCES proposals(id) ON DELETE CASCADE, name TEXT NOT NULL, \"websiteUrl\" TEXT, metrics JSONB, \"relativeScores\" JSONB, \"sortOrder\" INTEGER NOT NULL DEFAULT 0, \"createdAt\" TIMESTAMP NOT NULL DEFAULT now(), \"updatedAt\" TIMESTAMP NOT NULL DEFAULT now())"
do
...
(async () => {
  ...
  const services = [
    ...
    { name: 'Business Data Intelligence', shortDescription: 'Turn your data into actionable growth insights', description: 'We transform raw business data into clear, actionable intelligence.', outcomes: 'Data-driven decisions, performance visibility', deliverables: 'Analytics dashboard, custom reports', useCases: 'Scattered data, no visibility', pricingNotes: 'Setup from \$2,500', proofPoints: '28% faster decision-making', timeline: '3-5 weeks', sortOrder: 9 },
    { name: 'Customer Reactivation', shortDescription: 'Re-engage lost customers and recover revenue', description: 'We build automated reactivation campaigns that bring dormant customers back.', outcomes: 'Recovered revenue, reactivated customer base', deliverables: 'Reactivation strategy, automated campaigns', useCases: 'Large inactive lists, declining repeat purchases', pricingNotes: 'Setup from \$1,500', proofPoints: '18% reactivation rate on dormant lists', timeline: '2-3 weeks', sortOrder: 10 },
  ];
  ...
  const sections = [
    ...
    { title: 'Terms & Conditions', content: '1. This proposal is valid for 30 days from the date of issue.\n2. All prices are exclusive of applicable taxes.\n3. Payment terms: 50% upfront, 50% upon completion.\n4. Project timelines begin upon receipt of signed agreement.\n5. Scope changes documented via change order.\n6. IP transfers upon full payment.\n7. Both parties agree to confidentiality.\n8. 90-day warranty against defects.\n9. Cancellation with 14 days written notice.\n10. Governed by the laws of Nepal.', category: 'legal', sortOrder: 7 },
  ];
